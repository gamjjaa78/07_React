5//JSX 예제
import test1 from "../assets/세리카.jpg";
...
const Exam5_4=()=>{
    //JS 영역->if-else 사용가능
    return <h1>{true ? '환영!':'로그인 바람'}</h1>
}

//JSX 예제 5 : 배열을 이용한 화면 렌더링 방법(***중요:자주사용!!!)
//클라(React)<->서버(SpringBoot)
//서버에서 응답받은 데이터는 대부분 배열(List) 형태
const Exam5_5=()=>{
    //서버에서 아래 배열을 응답받았다고 가정
    const members=["짱구", "유리", "철수", "훈이", "맹구"]
    
    //JS 내장 함수 중 배열의 요소에 하나씩 순차접근해 콜백함수 내 내용수행 후
    //새로운 배열을 만들어 반환->map
    const listItm=members.map((member, index)=><li key={index}>{member}</li>);

    //Each child in a list should have a unique "Key" prop
    //->React에서 동적으로 list를 렌더링할때
    //각 자식 컴포넌트가 고유한 key 속성을 가져야한다는 규칙 위배시 발생하는 경고!
    //고유한 key값을 작성하면 경고 해결
    return <ul>{listItm}</ul>;
